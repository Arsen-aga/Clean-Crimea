class MultipleUploader {
  ...
  init({
    ...
    this.#multipleUploader.addEventListener("click", (e) => {
      if (
        e.target.dataset.msg === "multiple-uploader" ||
        e.target.dataset.msg === "mup-msg" ||
    ...
    this.#$imagesUploadInput.addEventListener("change", function () {
      if (this.files.length > 0) {
        ...
        const uploudImagesLength =
          self.#multipleUploader.querySelectorAll(".image-container").length;
        const allowedNumberOfUploadedFiles = maxUpload - uploudImagesLength;
        if (uploudImagesLength >= maxUpload) return;
        if (this.files.length > allowedNumberOfUploadedFiles) {
          this.files.splice(
            allowedNumberOfUploadedFiles,
            this.files.length - allowedNumberOfUploadedFiles
          );
        }
        self.#multipleUploader
          .querySelector(".mup-msg")
          .style.setProperty("display", "none"); // hide the hint texts inside drop zone
        self.#multipleUploader.classList.remove("block");
        self.#multipleUploader.classList.add("grid", "gap-6", "px-8");
  ...
  clear() {
    ...
    this.#multipleUploader.classList.add("block");
    this.#multipleUploader.classList.remove("grid", "gap-5", "px-8");
